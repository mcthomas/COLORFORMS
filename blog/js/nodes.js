var output = function (input) {
  ...
  input.setup = function () {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      nodes[i][3] = -0.5 + Math.random();
      nodes[i][4] = -0.5 + Math.random();
      nodes[i][5] = 0;
      nodes[i][6] = 0;
      nodes[i][7] = false;
      nodes[i][8] = false;
  ...
  var createNode = function () {
    ...
    nodes[nodes.length - 1][3] = -0.5 + Math.random();
    nodes[nodes.length - 1][4] = -0.5 + Math.random();
  ...
  var varyNodeSizes = function () {
    for (var i = 0; i < nodes.length; i++) {
      if (nodes[i][0] > 15) {
        ...
        nodes[i][0] += -0.1 + Math.random() * (0.2).toFixed(1);
      }
    }
  };
